...
The heart is responsible for promoting blood circulation, transporting oxygen and nutrients to various organs and tissues of the body, as well as carrying away waste and carbon dioxide from the body. Having a normal ejection fraction value is vital for human health. Echocardiography is the most common method of measuring ejection fraction but it is highly technological dependence. 4 ultrasound views are used to train the classification models. The knowledge distillation method is used to extract the knowledge from a VGG16 model and transfer it to small student models. The purpose of this project is to find an optimal student model with high accuracy, fast classification speed, and small model size so that it can perform optimally on mobile devices to help classify views and guide the correct image scanning. The four student models studied are Mobilenet-V3-small, Mobilenet-V3-large, Shufflenet, and Ghostnet. The result shows that MobileNet V3 Large had the highest relative validation accuracy at 0.98. smaller models, MobileNet V3 Small and ShuffleNet, are two smaller models and can achieve slightly higher inference speedups of 1.4 times.
